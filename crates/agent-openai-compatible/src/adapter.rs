...
use agent_model::ReasoningEffort;
...
/// max tokens 字段名等差异都通过 ProtocolAdapter 显式表达，测试可以直接用字面量构造方言。
...
impl ProtocolAdapter {
    ...
    ///
    /// 具体 Provider 的具名方言见 [`ProtocolAdapter::deepseek`]；调用方也可以用字面量
    /// 调整字段来构造自己的方言。
    ...
    ///   ProtocolAdapter：调用方经 `ModelRequest::provider_options` 的 `deepseek` 命名空间
    ///   传入，编码器按命名空间合并进请求根。
    ///   本 ProtocolAdapter 的响应校验始终按 thinking-enabled 语义执行；通过
