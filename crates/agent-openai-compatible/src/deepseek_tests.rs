...
use crate::{
    BearerCredential, ChatResponse, OpenAiCompatibleService, ProtocolAdapter, TransportTimeouts,
...
fn encode_pads_tool_call_assistant_without_reasoning_for_replay() {
    ...
    let encoded = encode_request(&request, &ProtocolAdapter::deepseek(), MODEL)
    ...
    // 占位是 DeepSeek ProtocolAdapter 方言行为，不能泄漏到普通 OpenAI-compatible 请求。
    let generic = encode_request(
        &request,
        &ProtocolAdapter::openai_compatible(provider_id("generic")),
        MODEL,
    )
...
async fn adapter_declares_deepseek_dialect_and_derives_capabilities() {
    let adapter = ProtocolAdapter::deepseek();
    ...
    // DeepSeek thinking 无强度档位映射（依据见 ProtocolAdapter::deepseek 文档注释）。
    assert_eq!(adapter.reasoning_effort_field, None);
    // thinking 模式下 temperature / top_p 不生效（官方文档，见 ProtocolAdapter::deepseek）。
...
fn flat_cache_usage_maps_only_for_deepseek_adapter() {
    ...
    let message =
        decode_response(&response, &ProtocolAdapter::deepseek()).expect("decode response");
    assert_eq!(
        message
            .usage
            .as_ref()
    ...
    let base = ProtocolAdapter::openai_compatible(provider_id("openai"));
