...
///
/// 参数是否受支持由 [`ProtocolAdapter`] 决定；请求设置了 ProtocolAdapter 不支持的参数时返回
/// [`ModelError::Config`]，不静默丢弃。`tool_choice` 例外于"原样透传"：`Auto`
/// 是线上的默认值，统一省略（语义相同，且兼容拒绝显式 `tool_choice` 的
/// Provider）；其余显式选择在 ProtocolAdapter 声明 `supports_tool_choice = false` 时
...
pub(crate) fn encode_request_with_images(
    ...
    // 工具列表中存在的名称。其余显式选择在 ProtocolAdapter 不支持时属于必然被拒的
    // 请求，编码期直接报 Config。
    ...
    // 只合并命名空间等于本 ProtocolAdapter Provider 的私有选项对象，其他命名空间忽略。
        ...
        match part {
            ...
            UserPart::FileReferences(files) => {
                ...
                for file in &files.files {
                    if let Some(image) = prepared_images.get(&file.readable_path) {
...
///
/// Reasoning parts 按序拼接进 ProtocolAdapter 声明的 reasoning 字段；Text parts 按序拼接为
/// content；ToolCall parts 按序进入 `tool_calls`。规范 Conversation 可能由另一个
/// Provider 方言产生；目标 ProtocolAdapter 未声明 reasoning 字段时，reasoning 作为不可移植的
/// 辅助内容在 wire 投影中省略，正文和工具交换仍按规范回放。`ProviderState` 不具备这种
/// 通用降级语义，仍然显式失败。
///
/// ProtocolAdapter 声明 [`ProtocolAdapter::tool_calls_require_reasoning`] 时（DeepSeek thinking 模式：
