...
///
/// 字段名随 ChatProtocolAdapter 变化的参数（max tokens、reasoning effort）和匹配命名空间的
...
pub struct ChatRequest {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    /// 采样温度；仅在 ChatProtocolAdapter 声明支持且请求设置时写入。
    pub temperature: Option<f32>,
    #[serde(skip_serializing_if = "Option::is_none")]
    /// nucleus 采样阈值；仅在 ChatProtocolAdapter 声明支持且请求设置时写入。
    pub top_p: Option<f32>,
    #[serde(skip_serializing_if = "Option::is_none")]
    /// 有序 stop 序列；仅在 ChatProtocolAdapter 声明支持且非空时写入。
    ...
    #[serde(flatten)]
    /// 字段名随 ChatProtocolAdapter 变化的参数与 Provider 私有选项，原样写入请求根。
...
pub struct ChatAssistantMessage {
    ...
    #[serde(flatten)]
    /// 随 ChatProtocolAdapter 变化的 reasoning 字段等额外字段。
    pub extra: BTreeMap<String, Value>,
}
...
pub struct ChatUsage {
    ...
    /// `prompt_cache_miss_tokens`），解码时按 [`crate::ChatProtocolAdapter::cached_input_tokens_field`]
    /// 声明的字段名查取，其余字段保留但不消费。
    pub extra: BTreeMap<String, Value>,
}
