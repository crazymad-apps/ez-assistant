...
impl ChunkAssembler {
    /// 创建按给定方言解码的组装器。
    pub fn new(adapter: ChatProtocolAdapter) -> Self {
        Self {
            adapter,
            turn: None,
            choice_index: None,
    ...
    pub fn push_chunk(&mut self, chunk: &ChatChunk) -> Result<Vec<ModelEvent>, ModelError> {
        ...
        if self.turn.is_none() {
            ...
            let model = ModelIdentity::new(self.adapter.provider.clone(), chunk.model.clone());
        ...
        let Some(choice) = chunk.choices.first() else {
            ...
            if let Some(usage) = &chunk.usage {
                let usage = map_usage(usage, &self.adapter);
