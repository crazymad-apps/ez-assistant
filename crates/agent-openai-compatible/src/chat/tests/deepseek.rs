...
//!
//! 离线契约测试用 `RecordedTransport` 回放 `fixtures/chat/deepseek/` 的可审阅 SSE
...
use crate::{
    BearerCredential, ChatProtocolAdapter, ChatResponse, OpenAiChatCompletionsService,
    TransportTimeouts, decode_response, encode_request,
};
...
const TURN_1_TRANSCRIPT: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/fixtures/chat/deepseek/turn_1_tool_call.sse"
));
/// 第二轮 transcript：reasoning + 最终回答 + 扁平 cache usage。
const TURN_2_TRANSCRIPT: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/fixtures/chat/deepseek/turn_2_final_answer.sse"
));
/// 预期的第二轮请求 JSON。
const TURN_2_REQUEST_JSON: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/fixtures/chat/deepseek/turn_2_request.json"
));
...
fn service_with(transport: &Arc<RecordedTransport>) -> OpenAiChatCompletionsService {
...
async fn turn_1_replays_reasoning_text_tool_call_and_cache_usage() {
    ...
    // thinking 开关按命名空间合并进请求根（M4 机制对 DeepSeek adapter 生效）。
...
fn encode_pads_tool_call_assistant_without_reasoning_for_replay() {
    ...
    let encoded = encode_request(&request, &ChatProtocolAdapter::deepseek(), MODEL)
    ...
    // 占位是 DeepSeek ChatProtocolAdapter 方言行为，不能泄漏到普通 OpenAI-compatible 请求。
    let generic = encode_request(
        &request,
        &ChatProtocolAdapter::openai_compatible(provider_id("generic")),
        MODEL,
    )
    .expect("generic adapter accepts the canonical tool exchange");
...
#[tokio::test]
async fn adapter_declares_deepseek_dialect_and_derives_capabilities() {
    ...
    assert_eq!(adapter.reasoning_effort_field, None);
    // thinking 模式下 temperature / top_p 不生效（官方文档，见 ChatProtocolAdapter::deepseek）。
    assert!(!adapter.supports_temperature);
    assert!(!adapter.supports_top_p);
    assert!(adapter.supports_stop);
    assert_eq!(
        adapter.max_output_tokens_field.as_deref(),
        Some("max_tokens")
    );
    assert_eq!(
        adapter.reasoning_replay_policy(),
        crate::ReasoningReplayPolicy::ToolCallsOnly
    );
    assert_eq!(
        adapter.cached_input_tokens_field.as_deref(),
...
fn thinking_mode_ineffective_generation_params_are_rejected() {
    ...
    assert!(matches!(
        encode_request(&request, &adapter, MODEL),
    ...
    let encoded =
        encode_request(&request, &adapter, MODEL).expect("stop and max_tokens are supported");
...
fn thinking_mode_explicit_tool_choice_is_rejected() {
    ...
    let encoded = encode_request(&request, &adapter, MODEL).expect("auto tool choice is omitted");
    let json = serde_json::to_value(&encoded).expect("serialize request");
...
#[test]
fn flat_cache_usage_maps_only_for_deepseek_adapter() {
    ...
    let message =
        decode_response(&response, &ChatProtocolAdapter::deepseek()).expect("decode response");
    assert_eq!(
        message
            .usage
            .as_ref()
    ...
    let base = ChatProtocolAdapter::openai_compatible(provider_id("openai"));
