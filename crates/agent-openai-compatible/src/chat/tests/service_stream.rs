...
use crate::{
    BearerCredential, ChatProtocolAdapter, OpenAiChatCompletionsService, SseFrame, SseParser,
};
...
/// DeepSeek thinking ChatProtocolAdapter 下偶发的工具调用响应：缺少 `reasoning_content`。
...
const CONTEXT_OVERFLOW_FRAME: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/fixtures/chat/errors/context_overflow_stream.sse"
));
...
/// 带 reasoning 字段的方言（DeepSeek 形态字面量；具名构造见 [`ChatProtocolAdapter::deepseek`]）。
...
fn service_with(
    transport: &Arc<RecordedTransport>,
...
) -> (OpenAiChatCompletionsService, Arc<RecordedTransport>) {
    let transport = Arc::new(RecordedTransport::new([Ok(RecordedResponse::new(
        200,
        body.into(),
    ))]));
    (service_with(&transport, reasoning_adapter()), transport)
}
...
async fn deepseek_stream_accepts_tool_call_without_reasoning() {
    ...
    let service = service_with(&transport, ChatProtocolAdapter::deepseek());
    let stream = service
        .stream(request(), ModelCallContext::default())
        .await
        .expect("stream established");
...
async fn stream_assembles_frames_split_across_body_chunks() {
    ...
    let service = service_with(&transport, reasoning_adapter());
    let stream = service
        .stream(request(), ModelCallContext::default())
        .await
        .expect("stream established");
...
#[tokio::test]
async fn capabilities_follow_the_adapter() {
    let transport = Arc::new(RecordedTransport::new([]));
    let with_reasoning = service_with(&transport, reasoning_adapter());
