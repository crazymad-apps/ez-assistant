...
fn lower_object(
    ...
    for (key, value) in object {
        match key.as_str() {
            ...
            // `properties` 的键是业务字段名，不是 Schema 关键字。必须只降级每个
            // property 的值，否则名为 `title`、`default` 等字段会被误当作元数据删除。
            "properties" => {
                lowered.insert(key.clone(), lower_properties(value, root, resolving_refs)?);
            }
            _ => {
    ...
    {
        // DashScope Responses rejects an otherwise valid empty object schema unless the
        // OpenAI-compatible `properties` member is present explicitly.
...
fn lower_properties(
    ...
    properties
        .iter()
        .map(|(name, schema)| {
            lower_schema(schema, root, resolving_refs).map(|schema| (name.clone(), schema))
        })
...
mod tests {
    ...
    #[test]
    fn compatible_subset_makes_empty_object_properties_explicit() {
        ...
        .expect("encode empty object");
        assert_eq!(encoded["properties"], serde_json::json!({}));
    }
    ...
    #[test]
    fn function_subset_preserves_property_names_that_match_schema_keywords() {
        let encoded = encode_tool_schema(
            &json!({
                "title": "ToolInput",
                "type": "object",
        ...
        .expect("keyword-shaped property names remain encodable");

        assert!(encoded.get("title").is_none());
        assert_eq!(encoded["properties"]["title"], json!({"type": "string"}));
        assert_eq!(encoded["properties"]["default"], json!({"type": "boolean"}));
        assert_eq!(encoded["required"], json!(["title"]));
    }
