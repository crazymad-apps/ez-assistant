...
use crate::{BearerCredential, OpenAiCompatibleService, ProtocolAdapter, SseFrame, SseParser};
...
/// DeepSeek thinking ProtocolAdapter 下偶发的工具调用响应：缺少 `reasoning_content`。
...
/// 带 reasoning 字段的方言（DeepSeek 形态字面量；具名构造见 [`ProtocolAdapter::deepseek`]）。
fn reasoning_adapter() -> ProtocolAdapter {
    ProtocolAdapter {
...
async fn deepseek_stream_accepts_tool_call_without_reasoning() {
    ...
    let service = service_with(&transport, ProtocolAdapter::deepseek());
    let stream = service
        .stream(request(), ModelCallContext::default())
        .await
        .expect("stream established");
