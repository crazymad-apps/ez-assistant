...
///
/// 字段名随 ProtocolAdapter 变化的参数（max tokens、reasoning effort）和匹配命名空间的
...
pub struct ChatRequest {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    /// 采样温度；仅在 ProtocolAdapter 声明支持且请求设置时写入。
    pub temperature: Option<f32>,
    #[serde(skip_serializing_if = "Option::is_none")]
    /// nucleus 采样阈值；仅在 ProtocolAdapter 声明支持且请求设置时写入。
    pub top_p: Option<f32>,
    #[serde(skip_serializing_if = "Option::is_none")]
    /// 有序 stop 序列；仅在 ProtocolAdapter 声明支持且非空时写入。
    ...
    #[serde(flatten)]
    /// 字段名随 ProtocolAdapter 变化的参数与 Provider 私有选项，原样写入请求根。
...
pub enum ChatUserContent {
    /// 纯字符串正文。
    Text(String),
    /// 有序 text/image part 数组。
    Parts(Vec<ChatContentPart>),
}

#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(tag = "type", rename_all = "snake_case")]
/// user 消息正文数组中的一个有序内容 part。
...
///
/// `reasoning_content` 等随 ProtocolAdapter 变化的 reasoning 字段由 `extra` 平铺承接，
/// 解码时再按 [`crate::ProtocolAdapter::reasoning_content_field`] 查取。
pub struct ChatAssistantMessage {
    ...
    #[serde(flatten)]
    /// 随 ProtocolAdapter 变化的 reasoning 字段等额外字段。
    pub extra: BTreeMap<String, Value>,
}
...
pub struct ChatUsage {
    ...
    /// `prompt_cache_miss_tokens`），解码时按 [`crate::ProtocolAdapter::cached_input_tokens_field`]
    /// 声明的字段名查取，其余字段保留但不消费。
    pub extra: BTreeMap<String, Value>,
}
