//! 调试查看器：网页端实时查看 LLM 数据流的独立开发工具。
//!
//! - [`wire`]：推送端与 server 之间的 JSON 线格式。
//! - [`client`]：出向推送客户端（fire-and-forget），供 CLI demo、将来的 Runtime 使用。
...
pub use wire::{BroadcastMessage, DEFAULT_PORT, DebugChannel, DebugEnvelope, DebugPayload, now_ms};
