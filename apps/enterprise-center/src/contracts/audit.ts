...
export class AuditQuery extends PageQuery {
  @ApiPropertyOptional({
    ...
    minimum: 1,
    maximum: 2147483647,
    description: '按审计记录 ID 精确查询，供详情路由直接加载',
  })
  readonly id?: number;
  @ApiPropertyOptional({ type: String, format: 'date-time', description: 'UTC ISO 8601 起始时间（含），须以 Z 结尾' })
  readonly from?: string;
  @ApiPropertyOptional({ type: String, format: 'date-time', description: 'UTC ISO 8601 结束时间（不含），须以 Z 结尾' })
  readonly to?: string;
  @ApiPropertyOptional({ type: 'integer', format: 'int32', minimum: 1, maximum: 2147483647 })
...
export class AuditDetails {
  @ApiPropertyOptional({ type: String, format: 'uuid' }) call_id?: string;
  @ApiPropertyOptional({ enum: ['request', 'response'] }) side?: 'request' | 'response';
  @ApiPropertyOptional({ type: Boolean }) enabled?: boolean;
  @ApiPropertyOptional({ type: Number }) content_retention_days?: number;
  @ApiPropertyOptional({ type: Number }) index_retention_days?: number;
  @ApiPropertyOptional({ type: String, format: 'uuid' }) provider_instance_id?: string;
  @ApiPropertyOptional({ type: String }) model_id?: string;
  @ApiPropertyOptional({ type: Number, minimum: 0 }) model_count?: number;
  @ApiPropertyOptional({ type: Number, minimum: 0 }) count?: number;
  @ApiPropertyOptional({ type: ModelSelection, nullable: true }) default_model?: ModelSelection | null;
  @ApiPropertyOptional({ type: AuditUserFields }) before?: AuditUserFields;
  @ApiPropertyOptional({ type: AuditUserFields }) after?: AuditUserFields;
}
export class AuditRecord {
  ...
  @ApiProperty({ type: 'integer', format: 'int32', minimum: 1, maximum: 2147483647, nullable: true })
  readonly actor_user_id!: number | null;
  @ApiProperty({ type: 'integer', format: 'int32', minimum: 1, maximum: 2147483647, nullable: true })
  readonly target_user_id!: number | null;
  @ApiProperty({ type: String, nullable: true, description: '关联操作者的登录用户名；未认证或关联用户不存在时为空' })
  readonly actor_username!: string | null;
  @ApiProperty({ type: String, nullable: true, description: '关联目标用户的登录用户名；无目标或关联用户不存在时为空' })
