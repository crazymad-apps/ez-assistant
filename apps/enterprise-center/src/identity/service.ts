...
import type { LoginResponse, IdentityResponse, IdentityUser, LoginRequest, PasswordRequest } from '../contracts/identity.js';
...
function publicUser(user: IdentityUser): IdentityUser {
  return { id: user.id, username: user.username, display_name: user.display_name, role: user.role,
    is_super_admin: user.is_super_admin, enabled: user.enabled };
}
...
export class IdentityService {
  ...
  onModuleDestroy(): void { this.tokens.clear(); this.llmKeys.clear(); }
  ...
  write<T>(work: (manager: EntityManager) => Promise<T>, committed: (result: T) => void = () => {}): Promise<T> {
    const pending = this.pendingWrite.then(async () => {
      const source = this.source, runner = source.createQueryRunner();
      ...
      try {
        ...
        try { if (runner.isTransactionActive) await runner.rollbackTransaction(); }
          ...
          try { await source.destroy(); } catch { /* 保留原始失败，对外由全局错误层脱敏。 */ }
        }
        throw error;
  ...
  async audit(manager: EntityManager, action: string, requestId: string, userId: number | null,
  ...
  /** 仅在身份事务成功提交后调用；角色/启停/重置与本人改密共用撤销口径。 */
  ...
  async login(input: LoginRequest, requestId: string): Promise<LoginResponse> {
    ...
    return this.write(async client => {
      ...
      if (user.password_hash !== before.password_hash || user.role !== before.role || user.is_super_admin !== before.is_super_admin) {
      ...
      const token = `ct_${randomBytes(32).toString('hex')}`, llm_key = `cl_${randomBytes(32).toString('hex')}`;
  ...
  async llmIdentity(key: string): Promise<IdentityResponse> { return this.me(this.llmKeys.get(key)); }
  ...
  async changePassword(token: string | undefined, input: PasswordRequest, requestId: string): Promise<void> {
    ...
    if (!await this.passwords.verify(input.old_password, before.password_hash)) throw new IdentityError(401, 'AUTH_FAILED');
    const hash = await this.passwords.hash(input.new_password);
    await this.write(async client => {
      const current = await this.userForToken(client, token);
      if (current.password_hash !== before.password_hash || current.role !== before.role || current.is_super_admin !== before.is_super_admin) {
        throw new IdentityError(409, 'AUTH_STATE_CHANGED');
      }
      await users.password(client, current.id, hash);
      await this.audit(client, 'self_password_changed', requestId, current.id);
      return current.id;
    }, id => this.revokeUser(id));
  }
}
