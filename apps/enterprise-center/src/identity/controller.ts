...
@ApiDefaultResponse({
  ...
  description: '400 输入无效；401 身份失败；403 来源/权限拒绝；409 状态变化；429 限流；503 服务不可用',
})
export class IdentityController {
  ...
  @ApiOperation({
    operationId: 'login',
    summary: '统一登录，返回内存 Token 和关联 LLM key；无 Cookie 操作',
    security: [],
  })
  ...
  @Post('logout')
  @HttpCode(204)
  @Access('optional', 'logout')
  @ApiOperation({
    operationId: 'logout',
    summary: '仅废弃当前 Token 和关联 key；缺失/无效/重复退出均成功；请求体为 {}',
  ...
  @Post('password')
  @HttpCode(204)
  @Access('authenticated', 'self_password_changed')
  @ApiOperation({
    operationId: 'changeOwnPassword',
    summary: '验证原密码并修改本人密码，保留已有 Token/key；不是管理重置',
  })
  @ApiBody({ type: PasswordRequest })
  @ApiNoContentResponse()
