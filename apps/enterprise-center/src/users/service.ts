...
const response = (row: UserRow): UserRecord => ({
  id: row.id,
...
const details = (row: Pick<UserRecord, 'display_name' | 'role' | 'enabled'>) => ({
  display_name: row.display_name,
  role: row.role,
  enabled: row.enabled,
});
...
export class UsersService {
  ...
  private get manager(): EntityManager {
    if (!this.connection) throw new IdentityError(503, 'SERVICE_UNAVAILABLE');
    return this.connection.manager;
  }

  private async target(client: EntityManager, id: number): Promise<UserRow> {
    const row = await users.byId(client, id);
    if (!row) throw new IdentityError(404, 'USER_NOT_FOUND');
    return row;
  }
  ...
  async create(token: string | undefined, input: CreateUserRequest, requestId: string): Promise<UserRecord> {
    ...
    return this.identity.write(async (client) => {
      ...
      await this.identity.audit(client, 'user_created', requestId, actor.id, undefined, row.id, {
        after: details(row),
      });
      return response(row);
    });
  ...
  async update(
    ...
    const result = await this.identity.write(
      async (client) => {
        const actor = await this.identity.requireAdmin(client, token);
        const before = await this.target(client, id),
          after = { ...before, ...input };
        ...
        const changed =
          before.display_name !== after.display_name || before.role !== after.role || before.enabled !== after.enabled;
        if (!changed) return { user: response(before), revoke: false };
        const row = (await users.update(client, id, after))!;
        await this.identity.audit(client, 'user_updated', requestId, actor.id, undefined, id, {
      ...
      (result) => {
        if (result.revoke) this.identity.revokeUser(id);
      },
    );
    return result.user;
  }
