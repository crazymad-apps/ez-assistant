...
export class UsersController {
  ...
  list(@Query() query: UserQuery, @Req() request: AuthenticatedRequest) {
    return this.identity.attempt(
      'users_read',
      request.id,
      () => this.users.list(userQuery(query)),
      request.identity!.user.id,
    );
  }

  @Post()
  @Access('admin', 'user_created')
  @ApiOperation({ operationId: 'createUser', summary: '创建普通用户或管理员，不允许传入超级管理员标记' })
  @ApiBody({ type: CreateUserRequest })
  @ApiCreatedResponse({ type: UserRecord })
  create(@Body() body: unknown, @Req() request: AuthenticatedRequest) {
    return this.identity.attempt(
      'user_created',
      request.id,
      () => this.users.create(request.token, createUserInput(body), request.id),
      request.identity!.user.id,
    );
  }

  @Patch(':id')
  @Access('admin', 'user_updated')
  @ApiOperation({
    operationId: 'updateUser',
    summary: '编辑显示名称、角色或启用状态，至少一项；不允许改账号/超级管理员标记',
    description:
  ...
  @ApiOkResponse({ type: UserRecord })
  update(@Param('id') id: string, @Body() body: unknown, @Req() request: AuthenticatedRequest) {
    return this.identity.attempt(
      'user_updated',
      request.id,
      () => this.users.update(request.token, recordId(id), updateUserInput(body), request.id),
      request.identity!.user.id,
    );
  }
  ...
  @HttpCode(204)
  @Access('admin', 'user_password_reset')
  @ApiOperation({
    operationId: 'resetUserPassword',
    summary: '管理员为任意用户重置密码（包括自己和超级管理员），不校验原密码；保留目标已有 Token/key',
  })
  @ApiParam({ name: 'id', schema: { type: 'integer', format: 'int32', minimum: 1, maximum: 2147483647 } })
  ...
  reset(@Param('id') id: string, @Body() body: unknown, @Req() request: AuthenticatedRequest) {
    return this.identity.attempt(
      'user_password_reset',
      request.id,
      () => this.users.resetPassword(request.token, recordId(id), resetPasswordInput(body), request.id),
      request.identity!.user.id,
    );
  }
}
