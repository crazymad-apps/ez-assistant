...
if (!url || !adminUrl || !restore || process.env.CENTER_TEST_BASELINE_BACKUP_VERIFIED !== 'true') throw new Error('须显式提供已核验隔离库及恢复备份');
...
if (target.hostname !== '127.0.0.1' || target.port !== '55432' || target.username !== 'center_app'
  || !/^\/ez_center_c01_test_m1_[a-z0-9_]+$/.test(target.pathname) || adminTarget.host !== target.host
  || adminTarget.username !== 'postgres' || adminTarget.pathname !== '/postgres'
  || !/^ez_center_c01_restore_m1_[a-z0-9_]+$/.test(restore)) throw new Error('不是已确认的 M1 隔离目标');
...
async function snapshot(pool: ReturnType<typeof createPool>) {
  ...
  for (const table of Object.keys(baselineCounts)) result[table] = (await pool.query(`SELECT * FROM public.${table} ORDER BY 1`)).rows;
  return result;
}
beforeAll(async () => {
  ...
  try { expect(await snapshot(restored)).toEqual(await snapshot(base)); } finally { await restored.end(); }
});
afterAll(async () => {
...
async function fixture() {
  ...
  const app = await createHttpApp({ source, origin }); apps.push(app);
  return { pool, app, uri };
}
...
async function host(f: Fixture, username = 'admin'): Promise<LoginResponse> {
  const response = await login(f, username); expect(response.statusCode).toBe(200); return response.json<LoginResponse>();
}
...
async function logout(f: Fixture, token: string) {
  return f.app.inject({ method: 'POST', url: '/api/auth/logout', headers: { authorization: `Bearer ${token}` }, payload: {} });
}
async function password(f: Fixture, token: string, old_password = '123456', new_password = 'new-password-123') {
  return f.app.inject({ method: 'POST', url: '/api/auth/password', headers: { authorization: `Bearer ${token}` }, payload: { old_password, new_password } });
}
async function ordinary(f: Fixture, enabled = true) {
  await f.pool.query(`INSERT INTO users(username,display_name,role,is_super_admin,enabled,password_hash)
    SELECT 'alice','Alice','user',false,$1,password_hash FROM users WHERE username='admin'`, [enabled]);
...
it('登录凭据仅在内存，数据库无会话表，me 不返回秘密，LLM key 不可用于身份 API', async () => {
  const f = await fixture(), result = await host(f);
  expect(result.token).toMatch(/^ct_[a-f0-9]{64}$/); expect(result.llm_key).toMatch(/^cl_[a-f0-9]{64}$/);
  ...
  expect(stored).not.toContain(result.token); expect(stored).not.toContain(result.llm_key);
  const contract = JSON.parse(readFileSync('tests/fixtures/identity/host-login.json', 'utf8'));
  expect({ ...result, center_id: '00000000-0000-4000-8000-000000000001',
    user: { ...result.user, id: 1 },
    token: 'ct_' + '0'.repeat(64), llm_key: 'cl_' + '0'.repeat(64) }).toEqual(contract);
});

it('普通用户与管理员统一 Bearer 登录；Cookie 不参与鉴权且后端不设置 Cookie', async () => {
  const f = await fixture(); await ordinary(f);
  for (const username of ['admin', 'alice']) {
    ...
    expect((await f.app.inject({ method: 'GET', url: '/api/auth/me', headers: { cookie: `token=${result.token}`, origin } })).statusCode).toBe(401);
    expect((await f.app.inject({ method: 'GET', url: '/api/auth/me', headers: { cookie: 'theme=dark', authorization: `Bearer ${result.token}`, origin } })).statusCode).toBe(200);
    const out = await logout(f, result.token);
    expect(out.statusCode).toBe(204); expect(out.headers['set-cookie']).toBeUndefined();
  }
});

it('未知账号、错误密码和停用账号统一失败，审计不保存输入内容', async () => {
  const f = await fixture(); await ordinary(f, false);
  for (const [username, pwd] of [['missing', 'secret-wrong'], ['admin', 'secret-wrong'], ['alice', '123456']]) {
    const response = await login(f, username, pwd);
    expect(response.statusCode).toBe(401); expect(response.json().error.code).toBe('AUTH_FAILED');
    expect({ error: { ...response.json().error, request_id: '00000000-0000-4000-8000-000000000004' } })
      .toEqual(JSON.parse(readFileSync('tests/fixtures/identity/auth-failed.json', 'utf8')));
    expect(response.body).not.toContain(pwd!);
  }
  const audit = (await f.pool.query("SELECT actor_user_id,target_user_id,reason_code,details FROM management_audit WHERE action='login'")).rows;
  expect(audit).toHaveLength(3);
  expect(audit.every(row => row.actor_user_id === null && row.target_user_id === null && row.reason_code === 'AUTH_FAILED')).toBe(true);
  expect(JSON.stringify(audit)).not.toMatch(/missing|secret-wrong|123456/);
});

it('多次登录秘密不同；退出只撤销当前 Token 和关联 key，重复退出不重复审计', async () => {
  const f = await fixture(), a = await host(f), b = await host(f);
  expect(a.token).not.toBe(b.token); expect(a.llm_key).not.toBe(b.llm_key);
  ...
  expect((await me(f, a.token)).statusCode).toBe(401); expect((await me(f, b.token)).statusCode).toBe(200);
  ...
  expect((await f.pool.query("SELECT count(*) FROM management_audit WHERE action='logout' AND success")).rows[0].count).toBe('1');
});

it('普通用户本人改密验证原密码，失败不撤销，成功撤销全部登录态且不影响其他人', async () => {
  const f = await fixture(); await ordinary(f);
  const a = await host(f, 'alice'), b = await host(f, 'alice'), administrator = await host(f);
  const failed = await password(f, a.token, 'wrong'); expect(failed.statusCode).toBe(401);
  expect((await me(f, a.token)).statusCode).toBe(200);
  expect((await password(f, a.token)).statusCode).toBe(204);
  for (const session of [a, b]) expect((await me(f, session.token)).statusCode).toBe(401);
...
it('管理员本人改密同样要求原密码', async () => {
  ...
  expect(changed.statusCode).toBe(204); expect(changed.headers['set-cookie']).toBeUndefined();
  expect((await me(f, result.token)).statusCode).toBe(401);
});

it('未知字段、错误类型、非 JSON 与来源伪造不签发凭据', async () => {
  const f = await fixture();
  for (const payload of [{ username: 'admin', password: '123456', is_super_admin: true }, { username: 'admin', password: '123456', delivery: ['host'] }]) {
    expect((await f.app.inject({ method: 'POST', url: '/api/auth/login', payload })).statusCode).toBe(400);
  }
  expect((await f.app.inject({ method: 'POST', url: '/api/auth/login', headers: { origin: 'https://evil.example' }, payload: { username: 'admin', password: '123456' } })).statusCode).toBe(403);
  expect((await f.app.inject({ method: 'POST', url: '/api/auth/login', headers: { 'content-type': 'text/plain' }, payload: 'secret' })).statusCode).toBe(400);
});

it('审计故障不签发 Token，回滚改密并保留已有 Token，不返回秘密或伪成功', async () => {
  const f = await fixture(), session = await host(f);
  const before = await snapshot(f.pool);
  // 已验证备份的独立夹具内制造固定审计错误，绝不作用于源/恢复库。
  await f.pool.query(`CREATE FUNCTION public.reject_audit() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'intentional audit failure'; END $$;
    CREATE TRIGGER reject_audit BEFORE INSERT ON management_audit FOR EACH ROW EXECUTE FUNCTION public.reject_audit();`);
  const refused = await login(f); expect(refused.statusCode).toBe(503); expect(refused.body).not.toContain('intentional');
...
it('受控交错：密码计算后账号停用，提交前重验拒绝签发', async () => {
  ...
  const ready = new Promise<void>(resolve => { computed = resolve; }), gate = new Promise<void>(resolve => { resume = resolve; });
  const original = Passwords.prototype.verify;
  const spy = vi.spyOn(Passwords.prototype, 'verify').mockImplementation(async function (this: Passwords, password, encoded) {
    const result = await original.call(this, password, encoded); computed(); await gate; return result;
  });
  const pending = login(f, 'alice');
  try {
    await ready;
    const c = await f.pool.connect();
    try { await c.query('BEGIN'); await c.query("UPDATE users SET enabled=false WHERE username='alice'"); await c.query('COMMIT'); }
    finally { c.release(); }
...
it('受控交错：新密码计算期间退出，改密不能使用旧的认证结果提交', async () => {
  ...
  const spy = vi.spyOn(Passwords.prototype, 'hash').mockImplementation(async function (this: Passwords, password) {
    const result = await original.call(this, password); computed(); await gate; return result;
  });
  const pending = password(f, session.token);
  try { await ready; expect((await logout(f, session.token)).statusCode).toBe(204); }
  finally { resume(); spy.mockRestore(); }
  expect((await pending).statusCode).toBe(401);
...
it('重建 HTTP 与数据库连接池后，旧 Token 和 key 全部失效', async () => {
  ...
  await f.app.close();
  const restartedPool = createPool({ url: f.uri.href }); pools.push(restartedPool);
  const source = await createDataSource({ url: f.uri.href }).initialize(); sources.push(source);
  const restarted = await createHttpApp({ source, origin }); apps.push(restarted);
...
it('入口限流忽略伪造转发地址，超过额度拒绝且不执行密码计算', async () => {
  const f = await fixture();
  for (let i = 0; i < 20; i++) expect((await f.app.inject({ method: 'POST', url: '/api/auth/login', headers: { 'x-forwarded-for': `192.0.2.${i}` }, payload: {} })).statusCode).toBe(400);
  const spy = vi.spyOn(Passwords.prototype, 'verify');
  try { expect((await login(f)).statusCode).toBe(429); expect(spy).not.toHaveBeenCalled(); } finally { spy.mockRestore(); }
});
...
it('实际产品进程重启后要求重新登录，不设置 Cookie', async () => {
  ...
  const address = server.address(); if (!address || typeof address === 'string') throw new Error('没有端口');
  const port = address.port;
  await new Promise<void>((resolve, reject) => server.close(error => error ? reject(error) : resolve()));
  const baseUrl = `http://127.0.0.1:${port}`;
  async function processService() {
    const child = spawn(process.execPath, ['dist/main.js', '--api-only'], { env: { ...process.env,
      CENTER_DATABASE_URL: f.uri.href, CENTER_DATABASE_AUTO_UPGRADE: 'false', CENTER_HOST: '127.0.0.1', CENTER_PORT: String(port),
      CENTER_PUBLIC_ORIGIN: baseUrl, CENTER_ALLOW_HTTP_LOOPBACK: 'true' }, stdio: ['ignore', 'pipe', 'pipe'] });
    const exited = new Promise<number | null>((resolve, reject) => { child.once('exit', resolve); child.once('error', reject); });
    await new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => { child.kill('SIGTERM'); reject(new Error('产品启动超时')); }, 3000);
      child.once('error', error => { clearTimeout(timer); reject(error); });
      child.stdout.on('data', chunk => { if (String(chunk).includes('center_listening')) { clearTimeout(timer); resolve(); } });
    });
    return async () => { child.kill('SIGTERM'); expect(await exited).toBe(0); };
  }
  const stopFirst = await processService();
  let token = '';
  try {
    const response = await fetch(`${baseUrl}/api/auth/login`, { method: 'POST', headers: { 'content-type': 'application/json', origin: baseUrl },
