...
function fixture(mode?: 'public' | 'optional' | 'admin', method = 'GET') {
  ...
  const context = { getHandler: () => function handler() {}, getClass: () => Endpoint,
    switchToHttp: () => ({ getRequest: () => request }) } as unknown as ExecutionContext;
  const identity = new IdentityService();
  const user = { id: 1, username: 'alice', display_name: 'Alice', enabled: true, role: 'user' as const, is_super_admin: false };
...
it.each(['public', 'optional'] as const)('%s 放行认证但不跳过来源与 JSON 检查', async mode => {
  ...
  const post = fixture(mode, 'POST'); post.request.headers = { origin };
...
it('管理员规则读取用户权限，超级管理员标记同样生效', async () => {
  const f = fixture('admin'); f.request.headers.authorization = 'Bearer valid';
