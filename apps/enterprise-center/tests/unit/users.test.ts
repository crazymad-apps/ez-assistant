...
const input = { username: ' Alice ', display_name: ' Alice ', role: 'user', enabled: true, password: 'safe-password-123' };
...
it('创建账号归一化；不接受标记、ID 或未知字段', () => {
  ...
  for (const extra of [{ is_super_admin: true }, { id: 'x' }, { token: 'x' }]) expect(() => createUserInput({ ...input, ...extra })).toThrow();
  expect(() => createUserInput({ ...input, password: '123456' })).toThrow();
});
it.each([{}, { username: 'other' }, { is_super_admin: false }, { enabled: 'false' }, { role: 'super' },
  { display_name: '   ' }, { display_name: 'bad\nname' }, { display_name: '界'.repeat(65) }])('编辑拒绝无效/越权字段 %j', body => {
...
it('PATCH 只修改显式字段，管理重置不收旧密码', () => {
  ...
  for (const value of ['0', '-1', '01', '1.0', '1e2', ' 1', '2147483648', '9007199254740993', 1, null, ['1'], 'AAAAAAAA-BBBB-CCCC-DDDD-EEEEEEEEEEEE']) expect(() => recordId(value)).toThrow();
});
it('用户查询只接受单值、有界分页和精确布尔值', () => {
  expect(userQuery({})).toEqual({ limit: 20, offset: 0 });
  expect(userQuery({ search: ' %_', role: 'admin', enabled: 'false', limit: '100', offset: '1000000' }))
    .toEqual({ search: '%_', role: 'admin', enabled: false, limit: 100, offset: 1000000 });
  for (const query of [{ limit: '0' }, { limit: '101' }, { limit: '1e2' }, { limit: ['1', '2'] }, { offset: '-1' },
    { offset: '1000001' }, { enabled: '1' }, { enabled: false }, { search: 'x'.repeat(65) }, { unknown: 'x' }]) expect(() => userQuery(query)).toThrow();
});
it('审计查询拒绝非法日期、倒序区间与重复参数', () => {
  expect(auditQuery({ from: '2026-09-18T00:00:00Z', to: '2026-09-19T00:00:00.000Z', success: 'true' })).toMatchObject({ success: true, limit: 20 });
  for (const query of [{ from: '2026-02-30T00:00:00Z' }, { to: 'not-date' },
    { from: '2026-09-18T00:00:00Z', to: '2026-09-18T00:00:00Z' }, { action: ['login'] }, { actor_user_id: 'x' }]) expect(() => auditQuery(query)).toThrow();
});
it('审计详情 ID 校验并参数化传入现有只读查询', async () => {
  ...
  const calls: unknown[][] = [];
  const manager = { query: async (...args: unknown[]) => { calls.push(args); return [{ total: 0, items: [] }]; } } as unknown as EntityManager;
...
it('审计详情只投影普通字段，不回传任意 JSON 或秘密', () => {
  expect(auditDetails({ password: 'secret', before: { display_name: 'Alice', role: 'user', password_hash: 'secret' },
...
it('OpenAPI 描述真实管理接口、分页字段和只读超级管理员标记', async () => {
  ...
  try {
    ...
    expect(document.components?.schemas?.AuditRecord).toMatchObject({ properties: {
      id: numericId, actor_user_id: { ...numericId, nullable: true }, target_user_id: { ...numericId, nullable: true },
      actor_username: { type: 'string', nullable: true }, target_username: { type: 'string', nullable: true },
    } });
    expect(document.paths['/api/users/{id}']?.patch?.parameters).toEqual(expect.arrayContaining([
      expect.objectContaining({ name: 'id', in: 'path', schema: numericId }),
    ]));
    ...
    expect(document.paths['/api/users']?.get?.parameters).toEqual(expect.arrayContaining([
      expect.objectContaining({ name: 'limit', in: 'query', required: false }), expect.objectContaining({ name: 'search', in: 'query' })]));
    expect(document.paths['/api/audit']?.get?.parameters).toEqual(expect.arrayContaining([expect.objectContaining({ name: 'actor_user_id' }), expect.objectContaining({ name: 'id', in: 'query', required: false })]));
    for (const schema of ['CreateUserRequest', 'UpdateUserRequest']) expect(document.components?.schemas?.[schema]).not.toHaveProperty('properties.is_super_admin');
