...
it('OpenAPI 在线与离线快照一致，只包含已实现接口', async () => {
  ...
  expect(Object.keys(document.paths)).toEqual(
    expect.arrayContaining([
      ...
      '/api/model-providers',
      '/api/runtime/model-configuration',
    ]),
  );
  expect(document.paths['/api/info']?.get).toMatchObject({
  ...
  expect(document.components?.schemas?.CenterInfo).toMatchObject({
    required: ['capabilities', 'software_version', 'protocol_version', 'min_protocol_version'],
    properties: {
      software_version: { type: 'string' },
      protocol_version: { type: 'integer', minimum: 1 },
      min_protocol_version: { type: 'integer', minimum: 1 },
    },
  });
