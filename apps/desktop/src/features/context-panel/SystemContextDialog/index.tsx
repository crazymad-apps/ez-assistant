...
export function SystemContextDialog(props: Readonly<{
  ...
  return (
    ...
    >
      <header>
        <div>
          <h2 id="system-context-title">System Context</h2>
          <p>当前会话创建时冻结的系统上下文原文，不会随全局配置变化。</p>
        </div>
        <div className={styles.header_actions}>
          <div aria-label="System Context 显示方式" className={styles.view_toggle} role="group">
            <button
      ...
      <div className={styles.body}>
        ...
        )) : <p className={styles.empty}>当前会话没有持久化的 System Context 内容。</p>}
      </div>
    </Dialog>
  );
}
