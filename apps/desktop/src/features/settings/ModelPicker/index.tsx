...
export function ModelPicker(props: ModelPickerProps) {
  ...
  const read_only =
    root.projection.application?.model_settings.management?.read_only ??
    settings.model_settings.management?.read_only ??
    false;
  const selection =
    read_only && props.follow_default
      ? (root.projection.application?.model_settings.default_model ?? settings.model_settings.default_model)
      : props.selection;
  const [result, setResult] = useState<Readonly<{
    provider_id: ProviderInstanceId;
    models: readonly DiscoveredModel[];
    fixed: readonly ModelFixedConfig[];
    loading: boolean;
    fixed_loading: boolean;
    error: string | null;
    fixed_error: string | null;
  }> | null>(null);
    ...
    void (async () => {
      try {
        ...
        if (!current.signal.aborted)
          setResult((value) =>
            value
              ? { ...value, loading: false, error: failure instanceof Error ? failure.message : '读取模型列表失败。' }
              : value,
          );
      }
    })();
        ...
        setResult((value) =>
          value
            ? {
                ...value,
                fixed_loading: false,
                fixed_error: failure instanceof Error ? failure.message : '读取已保存模型失败。',
              }
            : value,
        );
    }
  ...
  const categories: SettingsCascadeCategory[] = props.providers.map((provider) => {
    ...
    for (const fixed of current?.fixed ?? []) {
      ...
      models.set(id, { value: id, label: online?.label ?? id, description: online?.description });
    }
    return {
      id,
      label: provider.connection.display_name,
      value_label: '',
      hide_title: true,
      selected: selection?.provider_instance_id === id ? selection.model_id : '',
      options: [...models.values()].map((option) => ({ ...option, disabled: read_only })),
      on_open: () => {
        void load(id);
      },
      on_select: (model_id) =>
        read_only ? Promise.resolve(false) : props.onSelect({ provider_instance_id: id, model_id }),
      content:
        current &&
        (current.loading || current.fixed_loading || current.error || current.fixed_error || models.size === 0) ? (
          <>
            {current?.fixed_loading && <p role="status">正在读取已保存模型…</p>}
            {current?.fixed_error && (
              <p role="alert">
                {current.fixed_error}
                <Button type="button" onClick={() => void load(id)}>
                  重试读取
                </Button>
              </p>
            )}
            ...
            {current &&
              !current.loading &&
              !current.fixed_loading &&
              !current.error &&
              !current.fixed_error &&
              models.size === 0 && <p>暂无可用模型，请到模型管理页刷新在线模型。</p>}
          </>
        ) : null,
    };
  });
  ...
  return (
    <SettingsCascadePopover
      ...
      open={props.open}
      on_open_change={props.onOpenChange}
      clear_action={
        !read_only && props.clearable && props.selection
          ? { label: `清除${props.title}`, on_clear: () => props.onSelect(null) }
          : undefined
      }
      disabled={props.disabled}
      disabled_reason={props.disabled_reason}
      initial_category={props.initial_category ?? null}
      categories={categories}
      trigger_class_name={props.trigger_class_name}
      trigger_content={props.label}
      primary_actions={
        !read_only && props.follow_default
          ? [
              {
      ...
      primary_content={<>{props.providers.length === 0 && <p>暂无服务商</p>}</>}
      footer_content={
        props.show_manage_providers !== false && (
          <Button
            type="button"
            role="menuitem"
            onClick={() => {
