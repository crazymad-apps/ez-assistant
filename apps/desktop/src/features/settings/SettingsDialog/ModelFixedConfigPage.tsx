...
import {
  ...
  token_fields,
  type TokenDraft,
} from './modelSettingsValues';
import styles from './index.module.scss';

const efforts: readonly ReasoningEffortKey[] = ['low', 'medium', 'high', 'xhigh', 'max'];
export const ModelFixedConfigPage = observer(function ModelFixedConfigPage(
  props: Readonly<{
  ...
  const read_only = settings.model_settings.management?.read_only ?? false;
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setTokens(tokenDraft(result.parameters));
      setDirty(origin.current === 'manual' && result.source !== 'fixed');
  ...
  function change(patch: Partial<ModelParameters>) {
    setParameters((current) => (current ? { ...current, ...patch } : current));
    setDirty(true);
  }
  ...
  async function save() {
    ...
    try {
      desired = compileTokenDraft(parameters, tokens);
    } catch (failure: unknown) {
      setError(failure instanceof Error ? failure.message : '参数无效。');
      return;
    }
    setError(null);
    ...
    if (!applied) {
      ...
      try {
        ...
        if (!mounted.current) return;
        if (
          current.origin !== detail?.origin ||
          current.source !== 'fixed' ||
          !sameParameters(current.parameters, desired)
        )
          return;
        setDetail(current);
        settings.showNotice('已重新读取并确认固定配置。');
      } catch {
        return;
      }
    } else
      setDetail({
        ...
        source: 'fixed',
        updated_at_ms: applied.updated_at_ms,
        field_sources: {},
  ...
  async function refreshReference() {
    reference_request.current?.abort();
    const current = new AbortController();
    reference_request.current = current;
    setReference(null);
    ...
    try {
      ...
      if (!current.signal.aborted) setReferenceLoading(false);
    }
  }
  ...
  return (
    <SettingsPageContainer title={props.selection.model_id} on_back={props.onBack}>
      <SettingsMessages />
          ...
          <fieldset disabled={busy} className={styles.model_fieldset}>
              ...
              {token_fields.map((field) => (
                <label key={field.key}>
                  ...
                  <input
                    ...
                    placeholder="待配置，请核对文档"
                    onChange={(event) => {
                      setTokens({ ...tokens, [field.key]: event.target.value });
                      setDirty(true);
                    }}
                  />
                </label>
            ...
            <div className={styles.model_form}>
              ...
              <ModelFieldSelector
                disabled={busy || read_only}
                label="思考模式"
                value={parameters.reasoning_mode}
                options={[
                  { value: 'unknown', label: '未知' },
                  { value: 'unsupported', label: '不支持' },
                  { value: 'optional', label: '可开关' },
                  { value: 'always', label: '始终开启' },
                ]}
                onChange={(reasoning_mode) => change({ reasoning_mode })}
              />
            </div>
              ...
              <div className={styles.model_form}>
                {efforts.map((effort) => (
                  <label key={effort}>
                    ...
                    <input
                      disabled={read_only}
                      aria-label={`${effort} 线上值`}
                      maxLength={128}
                      placeholder="不支持"
                      value={parameters.reasoning_efforts?.[effort] ?? ''}
                      onChange={(event) => {
                        ...
                        change({
                          ...
                          default_reasoning_effort:
                            parameters.default_reasoning_effort && !values[parameters.default_reasoning_effort]
                              ? null
                ...
                <ModelFieldSelector
                  disabled={busy || read_only}
                  label="默认思考强度"
                  value={parameters.default_reasoning_effort ?? ''}
                  options={[
                    { value: '', label: '未设置' },
                    ...efforts
                      .filter((effort) => parameters.reasoning_efforts?.[effort])
                      .map((effort) => ({ value: effort, label: effort })),
                  ]}
                  onChange={(value) =>
                    change({ default_reasoning_effort: value === '' ? null : (value as ReasoningEffortKey) })
                  }
                />
              </div>
            </CollapsibleSection>
            <CollapsibleSection title="高级能力">
              <div className={styles.model_form}>
                {(['auto', 'none', 'required', 'named'] as const).map((key) => (
                  <ModelFieldSelector
                    ...
                    key={key}
                    label={`工具选择 ${key}`}
                    value={parameters.tool_choice[key]}
                    options={support_options}
                    onChange={(value) => change({ tool_choice: { ...parameters.tool_choice, [key]: value } })}
                  />
                ))}
                <ModelFieldSelector
                  disabled={busy || read_only}
                  label="工具图片传递"
                  value={parameters.tool_image_projection}
                  options={[
                    { value: 'unknown', label: '未知' },
                    { value: 'unsupported', label: '不支持' },
                    { value: 'native_tool_result', label: '工具结果原生图片' },
                    { value: 'follow_up_user_message', label: '后续用户消息图片' },
                  ]}
                  onChange={(tool_image_projection) => change({ tool_image_projection })}
                />
              </div>
            </CollapsibleSection>
            <p className={styles.model_documentation}>
              {document_url ? (
                ...
                <span>暂无可靠的厂商参考链接，请以服务商或本地部署设置为准。</span>
              )}
              {detail?.template_checked_on && (
                <span>模板核查：{detail.template_checked_on}；保存前请核对型号、接入范围与套餐。</span>
              )}
            </p>
            {!read_only && (
              <div className={styles.runtime_actions}>
                <Button
                  variant={detail?.origin === 'manual' ? 'danger' : 'outlined'}
                  type="button"
                  onClick={() => setResetting(true)}
                  disabled={busy || detail?.source !== 'fixed'}
                >
                  {detail?.origin === 'manual' ? '删除模型' : '重置配置'}
                </Button>
                <Button
                  type="button"
                  disabled={busy || dirty || (props.manual && detail?.source !== 'fixed')}
                  onClick={() => void settings.validateModel(props.selection)}
                >
                  {settings.pending_action === 'model:validate' ? '正在测试…' : '测试已保存配置'}
                </Button>
                <Button type="submit" disabled={busy || read_only}>
                  {settings.pending_action === 'model:save' ? '正在保存…' : '保存固定配置'}
                </Button>
              </div>
            )}
            {!read_only && detail?.source === 'fixed' && (
              <>
                <Button type="button" onClick={() => void refreshReference()}>
                  刷新在线参考
                </Button>
                {reference_loading && <p role="status">正在获取在线参考…</p>}
                {reference_error && <p role="alert">{reference_error}</p>}
                {reference && (
                  <>
                    <table className={styles.model_reference}>
                      <thead>
                        <tr>
                          <th>参数</th>
                          <th>当前草稿</th>
                      ...
                      <tbody>
                        {parameterRows(reference).map((row, index) => (
                          <tr key={row.label}>
                            <th>{row.label}</th>
                            <td>{current_rows[index]?.value}</td>
                            <td>{row.value}</td>
                          </tr>
                        ))}
                      </tbody>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setTokens(tokenDraft(reference));
                        setDirty(true);
                      }}
                    >
                      使用本次在线值
      ...
      {resetting && (
        <SessionActionDialog
          ...
          confirm_label={detail?.origin === 'manual' ? '删除模型' : '重置配置'}
          is_danger
          is_pending={busy}
          on_cancel={() => setResetting(false)}
          on_confirm={() => void reset()}
        >
          <p>
            {detail?.origin === 'manual'
              ? '将删除手动模型及其参数，并放弃未保存的修改。模型选择和历史保留；若在线列表仍有同 ID，后续作为在线模型使用，否则需重新选择。'
              : '将删除此模型的固定参数并放弃未保存的修改，恢复在线来源。接口缺少参数时需要重新填写；模型选择和历史保留。'}
          </p>
        </SessionActionDialog>
      )}
