...
export const ProviderSettingsPage = observer(function ProviderSettingsPage(
  props: Readonly<{
  ...
  const loadCatalog = useCallback(async () => {
    ...
    online_request.current = request;
    setCatalog(null);
    setError(null);
    setLoading(true);
    try {
  ...
  const loadFixed = useCallback(async () => {
    ...
    try {
      ...
      if (!request.signal.aborted) setFixedLoading(false);
    }
  }, [provider_id, settings]);
  useEffect(() => {
    void loadCatalog();
  ...
  async function loadUsage() {
    ...
    try {
      ...
      if (!request.signal.aborted) setUsageLoading(false);
    }
  }
  ...
  const offline_fixed = fixed.filter(
    (record) => !online_ids.has(record.selection.model_id) && matches(record.selection.model_id),
  );
  return (
    <SettingsPageContainer
      title={props.provider.connection.display_name}
      on_back={props.onBack}
      actions={
        !read_only && (
          <>
            <Button disabled={busy} type="button" onClick={props.onEdit}>
              编辑
            </Button>
            <Button
              variant="danger"
    ...
    >
      <SettingsMessages />
      <dl className={styles.model_details}>
        <dt>类型</dt>
        <dd>{provider_labels[props.provider.connection.provider_type]}</dd>
        <dt>服务地址</dt>
        <dd>{props.provider.connection.endpoint}</dd>
        <dt>凭据</dt>
        <dd>{read_only ? '由配置源管理' : props.provider.has_api_key ? '已设置' : '未设置'}</dd>
      </dl>
      <div className={styles.model_section_heading}>
        <h4>模型</h4>
        <div className={styles.runtime_actions}>
          {!read_only && (
            <>
              <Button onClick={props.onAddModel}>添加模型</Button>
              <Button disabled={refreshing} onClick={() => void refreshCatalog()}>
                {refreshing ? '正在刷新…' : '刷新在线模型'}
              </Button>
            </>
          )}
        </div>
      ...
      {error && (
        <p role="alert">
          {error}
          <Button onClick={() => void loadCatalog()}>重试读取</Button>
        </p>
      )}
      {!loading && catalog?.refreshed_at_ms == null && catalog?.diagnostic == null && (
        <p>暂无模型，可使用固定模型或点击“刷新在线模型”。</p>
      )}
      ...
      {fixed_error && (
        <p role="alert">
          {fixed_error}
          <Button onClick={() => void loadFixed()}>重试读取固定状态</Button>
        </p>
      )}
      <div className={styles.model_rows}>
        {visible.map((model) => (
          <ModelRow
            ...
            label={`配置模型 ${model.model_id}`}
            onSelect={() => props.onModel({ provider_instance_id: provider_id, model_id: model.model_id })}
          >
            <ModelStatusTags
              origin={fixed_origins.get(model.model_id) ?? 'online'}
              customized={fixed_origins.has(model.model_id)}
              configuration={model.configuration}
            />
          </ModelRow>
        ))}
        {offline_fixed.map((record) => (
          <ModelRow
            key={record.selection.model_id}
            model_id={record.selection.model_id}
            label={`编辑固定配置 ${record.selection.model_id}`}
            onSelect={() => props.onModel(record.selection)}
          >
      ...
      {deleting && (
        <SessionActionDialog
          title={`删除“${props.provider.connection.display_name}”？`}
          confirm_label="删除服务商"
          is_danger
          is_pending={settings.pending_action !== null}
          confirm_disabled={!usage || usage_loading}
          on_cancel={closeDeletion}
          on_confirm={props.onDelete}
        >
          {usage_loading && <p role="status">正在读取引用影响…</p>}
          {usage_error && (
            <p role="alert">
              {usage_error}
              <Button onClick={() => void loadUsage()}>重试</Button>
            </p>
          )}
          {usage && (
            <>
              <p>
                本次查询：{usage.session_count} 个显式引用会话（含归档）、{usage.fixed_config_count} 条固定配置。
              </p>
              {usage.default_model && <p>当前默认模型使用此服务商。</p>}
              {usage.vision_model && <p>辅助识图模型使用此服务商。</p>}
              {usage.sessions.length > 0 && (
                <ul>
                  {usage.sessions.map((session) => (
                    <li key={session.session_id}>{session.title}</li>
                  ))}
                </ul>
              )}
              {usage.session_count > usage.sessions.length && (
                <p>另有 {usage.session_count - usage.sessions.length} 个会话，删除会影响全部引用。</p>
              )}
              <p>影响数量可能随其他客户端操作变化，完成后以实际删除结果为准。</p>
            </>
          )}
          <p>
            将删除该服务商及其固定配置。已保存的模型选择和历史会保留，受影响的默认、辅助和会话模型需要重新选择。已经开始的执行使用原配置完成。
          </p>
        </SessionActionDialog>
      )}
    </SettingsPageContainer>
...
function ModelRow(
  ...
  return (
    <button type="button" aria-label={props.label} onClick={props.onSelect}>
      <span>
        <strong>{props.display_name ?? props.model_id}</strong>
