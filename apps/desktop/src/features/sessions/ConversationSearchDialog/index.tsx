...
function RecallMessage(props: Readonly<{ item: ConversationItem; is_anchor: boolean }>) {
  ...
  if (props.item.type === "shell_switch_result") {
    return <article className={styles.message} data-anchor={props.is_anchor} data-role="user"><small>Shell 切换结果</small><p>{props.item.success ? "切换完成" : "切换失败，已保留原 Shell"}</p></article>;
  }
