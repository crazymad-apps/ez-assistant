...
const effort_keys: readonly ReasoningEffortKey[] = ['low', 'medium', 'high', 'xhigh', 'max'];

/** 草稿没有 SessionView，菜单从 Runtime 的生效配置读取档位，复用模板与固定配置优先级。 */
export function useDraftModelEfforts(
  selection: ModelSelection | null,
  open: boolean,
): readonly ReasoningEffortOptionSnapshot[] {
  ...
  useEffect(() => {
    ...
    setLoaded(null);
    void store.settings
      .getModelConfiguration({ provider_instance_id: provider_id, model_id }, controller.signal)
      .then(({ parameters }) => {
        if (controller.signal.aborted) return;
        const options = effortOptions(parameters);
        setLoaded({ key: model_key, options });
      })
      .catch((error: unknown) => {
        if (!controller.signal.aborted)
          store.showInteractionError(error instanceof Error ? error.message : '读取模型强度失败，请重新打开菜单重试。');
      });
...
function effortOptions(parameters: ModelParameters): ReasoningEffortOptionSnapshot[] {
  const enabled =
    parameters.reasoning === 'supported' &&
