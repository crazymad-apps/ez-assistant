...
import {
  ...
  sameParameters,
  tokenDraft,
} from '../../src/features/settings/SettingsDialog/modelSettingsValues';
...
function detail(source: 'fixed' | 'online' = 'fixed'): ModelConfigurationDetail {
  return {
    ...
    updated_at_ms: source === 'fixed' ? 10 : null,
  };
}
...
describe('provider and fixed model settings', () => {
  it('uses the same provider, catalog and parameter views with a read-only source policy', async () => {
    const store = modelStore();
    store.settings.model_settings = {
      default_model: modelSelection,
      vision_model: null,
      management: { read_only: true, unavailable_reason: null, last_success_at_ms: 42, last_refresh_error: null },
    };
    vi.spyOn(store.settings, 'getModelConfiguration').mockResolvedValue(detail());
    show(store);
    expect(screen.getByRole('button', { name: '默认模型' })).toHaveTextContent(modelSelection.model_id);
    ...
    fireEvent.click(await screen.findByRole('button', { name: '配置模型 fixture' }));
    expect(await screen.findByLabelText('上下文窗口（Token）')).toBeDisabled();
    expect(screen.getByRole('button', { name: '高级能力' })).toBeEnabled();
    expect(screen.queryByRole('button', { name: '保存固定配置' })).not.toBeInTheDocument();
    expect(store.settings.getModelConfiguration).toHaveBeenCalledWith(
      modelSelection,
  ...
  it('ignores cancelled usage results when the deletion dialog is reopened', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '删除服务商' }));
    fireEvent.click(
      within(screen.getByRole('dialog', { name: '删除“测试服务商”？' })).getByRole('button', { name: '取消' }),
    );
    ...
    await act(async () => {
      finish({ default_model: false, vision_model: false, session_count: 99, fixed_config_count: 0, sessions: [] });
    });
    expect(dialog.getByRole('button', { name: '删除服务商' })).toBeDisabled();
  ...
  it('keeps credentials blank and preserves internal and outer navigation drafts', async () => {
    ...
    fireEvent.click(
      within(screen.getByRole('dialog', { name: '放弃未保存的修改？' })).getByRole('button', { name: '取消' }),
    );
    ...
    await waitFor(() =>
      expect(save).toHaveBeenCalledWith('provider-1', expect.objectContaining({ display_name: '新名称' }), {
        mode: 'unchanged',
      }),
    );
  ...
  it('opens fixed models independently when the local catalog read fails', async () => {
    ...
    vi.mocked(store.settings.listFixedModels).mockResolvedValue([
      { origin: 'online', selection: modelSelection, parameters: modelParameters(), updated_at_ms: 10 },
    ]);
    vi.spyOn(store.settings, 'getModelConfiguration').mockResolvedValue(detail());
    show(store);
    fireEvent.click(screen.getByRole('button', { name: /测试服务商 https/ }));
  ...
  it('leaves missing online limits blank and saves the complete user draft', async () => {
    ...
    const save = vi.spyOn(store.settings, 'saveModelFixedConfig').mockResolvedValue({
      origin: 'online',
      selection: modelSelection,
      parameters: modelParameters(),
      updated_at_ms: 11,
  ...
  it('keeps a successful reset distinct from a subsequent discovery failure', async () => {
    const store = modelStore();
    vi.spyOn(store.settings, 'getModelConfiguration')
      .mockResolvedValueOnce(detail())
      .mockRejectedValueOnce(new Error('online unavailable'));
    const reset = vi.spyOn(store.settings, 'resetModelFixedConfig').mockImplementation(async () => {
      store.settings.showNotice('固定配置已重置。');
      return true;
    });
    await openModel(store);
    ...
    fireEvent.click(
      within(screen.getByRole('dialog', { name: '重置固定配置？' })).getByRole('button', { name: '重置配置' }),
    );
  ...
  it('refreshes reference values without overwriting dirty or fixed values', async () => {
    ...
    vi.spyOn(store.settings, 'getModelConfiguration').mockResolvedValue(detail());
    await openModel(store);
    const online = discoveredModel();
    online.metadata.context_window_tokens = { state: 'known', value: 32000 };
    ...
    expect(table).toHaveTextContent('64000');
    expect(table).toHaveTextContent('32000');
    expect(screen.getByLabelText('上下文窗口（Token）')).toHaveValue('64000');
  ...
  it('requests only the opened provider and ignores results after close', async () => {
    ...
    store.settings.providers = [
      modelProvider,
      {
        ...
        connection: { ...modelProvider.connection, display_name: '另一服务商' },
      },
    ];
    let finish!: (models: ReturnType<typeof discoveredModel>[]) => void;
    vi.mocked(store.settings.listProviderModels).mockImplementation(
      () =>
        new Promise((resolve) => {
          finish = resolve;
        }),
    ...
    fireEvent.click(trigger);
    finish([discoveredModel('late-model')]);
    fireEvent.click(trigger);
...
describe('fixed parameter draft validation', () => {
  it('rejects fractional, unsafe and contradictory token limits', () => {
    ...
    const draft = tokenDraft(parameters);
    for (const value of ['1.5', '0', '-2', '9007199254740992'])
      expect(() => compileTokenDraft(parameters, { ...draft, context_window_tokens: value })).toThrow();
  ...
  it('does not equate invalid optional limits with unknown ones', () => {
    const left = modelParameters();
    const right = modelParameters();
    right.max_input_tokens = { state: 'invalid' };
