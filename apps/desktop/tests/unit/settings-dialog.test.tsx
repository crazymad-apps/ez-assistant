import { manageLocalHost } from '../../src/native-bridge/runtimeConnection';
...
vi.mock('../../src/native-bridge/runtimeConnection', async () => ({
  ...(await vi.importActual('../../src/native-bridge/runtimeConnection')),
  manageLocalHost: vi.fn(),
}));
...
describe('SettingsDialog model management', () => {
  it('shows managed model status and refresh without local editing controls', async () => {
    const store = settingsStore();
    store.settings.model_settings = {
      default_model: { provider_instance_id: 'managed', model_id: 'center-model' },
      vision_model: null,
      management: {
        read_only: true,
        unavailable_reason: null,
        last_success_at_ms: 42,
        last_refresh_error: '中心暂不可达',
      },
    };
    const refresh = vi.spyOn(store.settings, 'refreshModelSource').mockResolvedValue(true);
    renderDialog(store);
    expect(screen.getByRole('button', { name: '默认模型' })).toHaveTextContent('center-model');
    expect(screen.getByText('刷新失败：中心暂不可达')).toBeVisible();
    ...
    runInAction(() => {
      store.settings.model_settings = {
        ...
        management: {
          read_only: true,
          unavailable_reason: '管理员尚未配置默认模型。',
          last_success_at_ms: 43,
          last_refresh_error: null,
        },
      };
    });
    await waitFor(() => expect(screen.getByRole('status')).toHaveTextContent('管理员尚未配置默认模型。'));
    expect(screen.queryByText('center-model')).not.toBeInTheDocument();
  });
            ...
            server: expect.objectContaining({
              ...
              transport: {
                ...
                payload: {
                  ...
                  args: { mode: 'keep' },
                  cwd: { mode: 'keep' },
                  environment: { TOKEN: { mode: 'replace', value: 'new-secret' } },
                },
              },
            }),
          },
  ...
  it('shows long tool descriptions as non-blocking warnings in the list and connection test', () => {
    ...
    const diagnostic = {
      server_key: 'github',
      code: 'tool_description_long' as const,
      field_path: 'tools/batch_design/description',
      message: 'Tool batch_design description is 14045 bytes; retained in full and available',
    };
    store.settings.mcp.configuration = {
      ...store.settings.mcp.configuration!,
      needs_refresh: false,
      diagnostics: [diagnostic],
      servers: store.settings.mcp.configuration!.servers.map((server) => ({
        ...server,
        needs_refresh: false,
    ...
    store.settings.mcp.test_result = {
      outcome: 'success',
      stage: 'complete',
      elapsed_ms: 42,
      tool_count: 13,
  ...
  it('uses shared MCP field selectors and submits explicit replace and remove modes', async () => {
    ...
    fireEvent.click(
      within(screen.getByRole('listbox', { name: '工作目录 cwd修改方式' })).getByRole('option', { name: '移除' }),
    );
          ...
          payload: {
            server: expect.objectContaining({
              transport: {
                type: 'stdio',
                payload: {
                  command: { mode: 'replace', value: '/local/mcp-server' },
                  args: { mode: 'keep' },
                  cwd: { mode: 'remove' },
                  environment: { TOKEN: { mode: 'keep' } },
                },
              },
            }),
          },
  ...
  it('keeps HTTP URL redacted when its shared selector returns to keep', async () => {
    ...
    store.settings.mcp.configuration = {
      ...
      servers: store.settings.mcp.configuration!.servers.map((server) => ({
        ...
        target_summary: 'https://mcp.example.test',
        environment_keys: [],
    ...
    fireEvent.change(screen.getByRole('textbox', { name: '服务 URL' }), {
      target: { value: 'https://mcp.example.test/new' },
    });
    fireEvent.click(url_mode);
    fireEvent.click(
      within(screen.getByRole('listbox', { name: '服务 URL修改方式' })).getByRole('option', { name: '保持原值' }),
    );
          ...
          payload: {
            server: expect.objectContaining({
              transport: {
                ...
                payload: {
                  url: { mode: 'keep' },
                  headers: { Authorization: { mode: 'keep' } },
                },
              },
            }),
          },
  ...
  it('binds MCP deletion to the revision the user reviewed', async () => {
    ...
    await user.click(
      within(screen.getByRole('dialog', { name: '删除 MCP 服务？' })).getByRole('button', { name: '取消' }),
    );
    await waitFor(() => expect(delete_button).toHaveFocus());
    await user.click(delete_button);
    runInAction(() => {
      store.settings.mcp.configuration = { ...store.settings.mcp.configuration!, revision: 'mcp-r2' };
    });
    fireEvent.click(screen.getByRole('button', { name: /^删除服务$/ }));
    await waitFor(() =>
      expect(mutate).toHaveBeenCalledWith('mcp-r1', { type: 'remove', payload: { server_key: 'github' } }),
    );
  });
  ...
  it('hides MCP header deletion for new servers and disables it during editing operations', async () => {
    ...
    const delete_button = screen.getByRole('button', { name: '删除服务…' });
    runInAction(() => {
      store.settings.mcp.testing = true;
    });
    await waitFor(() => expect(delete_button).toBeDisabled());
    runInAction(() => {
      store.settings.mcp.testing = false;
      store.settings.pending_action = 'mcp:save';
    });
    await waitFor(() => expect(delete_button).toBeDisabled());
    runInAction(() => {
      store.settings.pending_action = null;
    });
  ...
  it('previews imports with same-name replacement unchecked by default', async () => {
    const store = mcpSettingsStore();
    vi.spyOn(store.settings.mcp, 'previewImport').mockResolvedValue({
      diagnostics: [],
      entries: [
        {
          server_key: 'github',
    ...
    fireEvent.change(screen.getByRole('textbox', { name: 'MCP 配置 JSON' }), {
    ...
    await waitFor(() =>
      expect(mutate).toHaveBeenCalledWith('mcp-r1', {
        type: 'import',
        payload: { document: '{"mcpServers":{}}', replace_server_keys: [] },
      }),
    );
  });
  ...
  it('groups Runtime pages under one sidebar entry and separates diagnostics from local controls', () => {
    ...
    expect(diagnostic_heading.parentElement).toHaveTextContent('诊断信息/private/runtime/config.toml');
    expect(screen.queryByRole('button', { name: '停止本机 Runtime' })).not.toBeInTheDocument();
    expect(within(navigation).getByRole('button', { name: 'Runtime' })).toHaveAttribute('aria-current', 'page');
  ...
  it('labels remote Host pages and preserves dirty access settings until navigation is confirmed', async () => {
    ...
    vi.mocked(manageLocalHost).mockImplementation(hostAccessCommand);
  ...
  it('hides the personal access password for an enterprise Host', async () => {
    vi.mocked(nativeCore.isTauri).mockReturnValue(true);
    const store = new RootStore({ target_kind: 'local' });
    store.settings.is_open = true;
    store.settings.page = 'host_access';
    store.connection.state = 'connected';
    vi.mocked(manageLocalHost).mockResolvedValue({
      revision: 'enterprise',
      mode: 'enterprise',
    ...
    renderDialog(store);
    expect(await screen.findByLabelText('企业中心地址')).toHaveValue('http://center.test');
    expect(screen.queryByRole('heading', { name: '访问密码' })).not.toBeInTheDocument();
    expect(screen.queryByLabelText('新密码')).not.toBeInTheDocument();
    expect(screen.getByRole('checkbox', { name: '允许其他设备连接' })).toBeEnabled();
  });
  ...
  it('saves IP and domain entries with a single port and requires explicit restart before remote access', async () => {
    ...
    const hostAccessCommand = vi
      ...
      .mockResolvedValueOnce({
        ...
        configuration: { ...configuration, port: 7241 },
      });
    const restart = vi.spyOn(store.desktop_lifecycle, 'request').mockImplementation(() => {});
    vi.mocked(manageLocalHost).mockImplementation(hostAccessCommand);
    vi.spyOn(store, 'runtime_client', 'get').mockReturnValue({
      address: 'http://127.0.0.1:7240',
      hostAccessCommand,
    ...
    fireEvent.change(screen.getByLabelText('允许的 IP 或域名（可选，每行一个）'), {
    ...
    expect(hostAccessCommand).toHaveBeenLastCalledWith({
      type: 'configure',
      payload: {
        expected_revision: 'one-port',
        mode: 'personal',
        center_url: undefined,
        clear_center_binding: false,
        configuration: { ...configuration, port: 7241, server_names: ['127.0.0.1', '::1', 'example.com'] },
      },
    });
  ...
  it('manages Gateway access and confirms a visible device candidate without CLI state', async () => {
    ...
    expect(
      screen.queryByText('本阶段只展示 Host 当前能力，不在这里编辑服务商、模型、声音或密钥。'),
  ...
  it('keeps skill row actions separate and locks toggles while a change is pending', async () => {
    ...
    runInAction(() => {
      store.settings.pending_skill_name = 'review';
    });
    await waitFor(() => {
  ...
  it('selects auxiliary vision through provider and online model levels', async () => {
    ...
    await waitFor(() =>
      expect(set_vision).toHaveBeenCalledWith({ provider_instance_id: 'provider-1', model_id: 'online-model' }),
    );
  });
  ...
  it('reloads online entries on reopening and does not restore them after failure', async () => {
    ...
    store.settings.providers = [onlineProvider()];
    const list = vi
      .spyOn(store.settings, 'listProviderModels')
      .mockResolvedValueOnce([onlineModel()])
...
describe('SettingsDialog permission management', () => {
  it.each(['tool', 'server', 'all'] as const)(
    'edits and round-trips %s MCP permissions using shared controls',
    async (scope) => {
      const store = settingsStore();
      ...
      await waitFor(() =>
        expect(save).toHaveBeenCalledWith(document.scope, document.revision, {
          schema_version: 1,
...
function applicationWithSession(session: SessionSummary): ApplicationSnapshot {
  return {
    ...
    providers: [],
    model_settings: { default_model: null, vision_model: null },
    workspaces: [],
...
function permissionDocument(scope: PermissionDocumentSnapshot['scope'], editable: boolean): PermissionDocumentSnapshot {
  return {
    scope,
...
function onlineProvider(): import('@ez-assistant/protocol').ProviderSummary {
  return {
    ...
    connection: {
      ...
      protocol_preference: 'chat_completions',
