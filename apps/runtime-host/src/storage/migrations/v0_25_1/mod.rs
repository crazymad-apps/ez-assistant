//! 固定 0.25.1 初始版本；仅在外层逐版本事务内运行，不猜测历史软件版本。
...
const REQUIRED_PROJECTIONS: &[&str] = &[
    "SELECT session_id, title, model_provider_instance_id, model_id, reasoning_effort, system_prompt_json, skill_catalog_json, current_variant, approval_mode, lifecycle, body_generation, message_count, created_at_ms, updated_at_ms, archived_at_ms, is_pinned, title_origin, pc_output_device_id, materialization_key, automatic_title_pending FROM sessions LIMIT 0",
...
fn apply(connection: &Connection) -> Result<()> {
    initialize_retained_schema(connection)?;
    ensure_column(
        connection,
        "sessions",
    ...
    ensure_column(
        ...
        "sessions",
        "model_id",
        "ALTER TABLE sessions ADD COLUMN model_id TEXT CHECK ((model_provider_instance_id IS NULL AND model_id IS NULL) OR (model_provider_instance_id IS NOT NULL AND length(model_provider_instance_id)>0 AND model_id IS NOT NULL AND length(model_id)>0))",
    )?;
    ...
    if old {
        connection.execute_batch("ALTER TABLE sessions DROP COLUMN model_key")?;
    }
    connection.execute_batch(include_str!("../../model_management/schema.sql"))?;
    Ok(())
}

fn initialize_retained_schema(connection: &Connection) -> StorageResult<()> {
    connection.execute_batch(SCHEMA).map_err(|source| {
    ...
    backfill_workspace_labels(connection)?;
    ensure_column(
        connection,
        "session_resources",
    ...
    for (column, migration) in [
        ...
        ensure_column(connection, "session_usage", column, migration)?;
    }
    connection
        .execute(
...
fn validate(connection: &Connection) -> Result<()> {
    ...
    connection.prepare("SELECT provider_instance_id,display_name,provider_type,endpoint,api_key,protocol_preference,models_path,discovery_format FROM providers LIMIT 0")?;
    connection.prepare("SELECT default_provider_instance_id,default_model_id,vision_provider_instance_id,vision_model_id FROM model_settings LIMIT 0")?;
    connection.prepare("SELECT provider_instance_id,model_id,context_window_tokens,max_output_tokens,max_input_tokens,mode_limits_json,capabilities_json,updated_at_ms,origin FROM model_fixed_configs LIMIT 0")?;
    let old: bool = connection.query_row(
        "SELECT EXISTS(SELECT 1 FROM pragma_table_info('sessions') WHERE name='model_key')",
        [],
        |r| r.get(0),
...
fn validate_queue_payloads(transaction: &Connection) -> StorageResult<()> {
...
fn backfill_workspace_labels(transaction: &Connection) -> StorageResult<()> {
    let rows = {
        let mut statement = transaction
            .prepare(
