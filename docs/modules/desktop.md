...
`apps/desktop` 是 Tauri 2 桌面应用，负责 WebView UI 与 macOS/桌面系统能力，并把 UI 意图适配到 `assistant-runtime`。
...
- Tauri command 参数适配、Runtime 事件转发和错误展示转换。
- 应用启动、隐藏、退出及 Runtime 初始化。

## 核心约束

- `src-tauri` 可以依赖 `assistant-runtime`，但 Tauri command 必须薄，不实现会话或 Agent Loop。
- WebView 不直接访问文件系统、模型 API、数据库或 Shell；必须通过受控 Tauri command。
- 关闭主窗口不得默认终止 Runtime。真正退出应用时应先触发受控关闭流程。
