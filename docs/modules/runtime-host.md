...
> C03 M0 更新：下文历史版本条目中的根 `config.toml`、个人 `data/` 等位置，由本页末尾的 v0.27.0 配置与用户域规则替代；其余既有职责继续有效。
...
- 只有路由注册处媒体组中的八条 GET 媒体／下载路由可复用所属登录声明；原生凭据无该回退。
...
## v0.27.0 C03 M0 配置与个人域基础

- 版本迁移统一放在 `storage/migrations/v<主>_<次>_<修订>/mod.rs`；该版本的测试、SQL 等配套文件同目录收纳，不在 `migrations/` 根同时放置同名版本入口文件。
- `host.toml` 是 Host 的布局版本、模式、中心配置和 `[host_access]` 的唯一持久来源；原生发现与实例锁仍在根 `run/`。用户业务配置和数据装配到 `users/_personal/`，不能继续读取根个人配置或另建替代库。
- 持有整个 Home 的实例锁后、监听前执行具体 `host_layout::upgrade`。缺少 host.toml 进入 v0.27.0 升级，空 Home 共用入口；先核验独立整套备份，逐项移动，最后发布 host.toml。版本未知、源目标冲突、损坏库或缺库但仍有配套数据时停止，重入依据固定备份和实际文件核对，不自动恢复覆盖。
- SQLite 定位字段、v0.27.0 账本和最低 Host 版本在同一事务提交；已提交 Conversation、冻结 Prompt、C02 历史及派生快照保持原内容。历史附件仅由本域 Store 核实后提供旧路径映射，已记录文件资源仍校验 owner 和 Session 目录范围；不扩展任意 Host 文件请求。
- 个人 Skill 主来源为 `_personal/skills`，根 `skills` 是低优先级补充；自定义 Home 保留原操作系统用户 `.ez-assistant/skills` 兼容来源。旧根 Skill 必须先迁入个人域，不能因升级变为公共资源。
- `access read` 只读预览，不创建 Home、布局或数据库；停服写操作先校验输入和 CAS，再持实例锁完成布局升级，写入 Host 配置。在线 Host 设置不触发用户模型配置重载。
- 构建变量 `EZ_ASSISTANT_DEFAULT_CENTER_URL` 由 Host build.rs 校验并内置。仅首次创建企业配置时使用，显式输入优先；换包保留已有中心地址/身份及模式，不读取目标机器同名环境变量作为配置覆盖。
- M0 尚未接入企业登录和用户 Runtime。企业模式启动明确拒绝业务装配，不得回落个人域；M1/M2 继续完成该入口。真实 Windows/Linux 安装升级和正式制品分发由 C06 验证。

## v0.27.0 C03 M1 企业身份

- `access/enterprise/` 私有适配现有 Center 身份协议；协议版本与软件版本分开核验，首次中心 UUID 经 Host 配置 actor/CAS 绑定。禁止重定向，普通核验 5 秒、登录/改密 15 秒、响应体 64 KiB；不返回中心原始错误或秘密。
- 复用 `Credentials` 登录表及 `AccessPermit`。每用户键（center_id/user_id）只保存当前 Center Token/key；Desktop/Web 新登录原子替换同一状态。网络不持锁，同凭据正在进行的核验共享 future，结果提交比较对象身份；取消最后一个核验请求即释放 future，不缓存授权结果。
- Host Token 仍为七天；自然过期只结束客户端访问。身份失效结束同用户全部 Host 登录，中心暂不可达关闭访问通道但保留凭据，恢复后重新核验。Host Supervisor 单一复核循环只关注活动访问；后台工作和 Runtime 收尾在 M2 接入，不能以本阶段声称已取消业务任务。
- 企业主动退出先清本机用户状态及全部相关 Host Token，再尝试撤销退出时捕获的当前 Center 凭据；中心失败不恢复本机状态。Cookie 被新登录替换时只结束旧浏览器登录；快捷兑换不执行新的 Center login。
- 企业 bootstrap 只做本机 Host 管理；native 登录参数只选择 Token 响应，不授予管理权限。身份、改密、退出、Host 配置独立于业务就绪。M1 企业 Host 健康只表示身份服务可用，不打开个人数据库；企业业务、文件和终端在后续用户路由接入前保持不可用。
- Host 模式由 capabilities 公开；本人身份响应使用 no-store，Web 继续 HttpOnly Cookie。login_context 由既有登录 ID 派生，不能作为凭据；企业 Cookie 写入已有边界核对，个人/企业正式客户端及资源/WS 上下文整合在 M3/M4 完成。

## v0.27.0 C03 M2 用户 Runtime 与生命周期

- `user_domain` 由 Host Supervisor 监督，按稳定用户键缓存现有 `ReadyServices`；同用户装配与关闭共用异步锁。个人根为 `users/_personal`，企业根为 `users/<center_uuid>_<user_id>`，Store、Runtime、MCP、Gateway、Speech 使用同一个共用装配函数，根目录不能被业务请求替换。
- 登录只建立身份。`POST /runtime/ensure-ready` 等待本人装配完成后返回 204；命令、快照和事件复用同一服务获取入口。管理操作不先初始化业务；停止 Host 的命令等待全部用户域收尾完成后才返回 stopped。
- 客户端断开不取消受监督的初始化，失败先收尾再允许重试；退出或 Host 停止后晚到的结果不能重新开放业务。部分资源也有明确 owner，关闭失败保留失败事实并阻止再次开库，不能用 Drop 冒充成功关闭。
- 主动退出取消本人 Runtime、关闭相关访问并等待回收，其他用户继续。客户端 Token 到期只结束相应访问，既有 Runtime 任务 owner 的工作通知和 Gateway 活动事实决定是否仍需保留资源；活动工作继续参与中心身份复核，无有效登录且工作结束后关闭企业用户域。个人本机 bootstrap 保持既有 Host 生命周期，不因普通 Token 到期失去本机管理入口；个人主动退出仍关闭当前业务域，后续本机访问经同一入口重新装配。
- 回收复用 Runtime 操作门禁复核工作与当前登录，避免接纳任务和关闭竞态。不新增空闲回收、任务级登录映射、用户域配额或第二份 Run 状态。Host 停止并行收尾用户域，保留既有总关闭期限。
- 中心暂不可达取消受影响用户的工作和连接、保留登录供显式复核恢复；就绪等待返回服务不可用，不误报登录无效。明确身份失效才结束该用户登录。Runtime/Core 不接收 Center Token/key，企业模型工厂统一返回配置未就绪且禁止个人模型编辑；真实代理归 C04。
- M2 只完成核心业务路由及用户服务装配。文件、终端、原生设备及工具路径的完整归属检查仍归 M3，正式 Desktop/Web 登录和退出确认归 M4，不能将当前阶段视为完整产品隔离验收。

## v0.27.0 C03 M3 资源入口与路径保护

- `http/routes.rs` 是 HTTP 路由与准入中间件的统一装配处；按用户资源、原生路径、媒体、账号与管理入口分组。`auth` 和 `compatibility` 不重复维护路径名单。来源与凭据解析在最外层，普通业务先完成登录、账号上下文与中心复核，再检查版本并获取本人服务；退出不依赖中心复核或版本准入，终端仍在有界首帧中完成认证与版本检查。

- HTTP 资源路由在认证后等待并固定本用户 ReadyServices；上传暂存使用用户根的 `data/staging/uploads`，预览、下载、导出与原生路径在本人 Runtime 校验完整资源归属。业务流随访问凭据或用户域结束关闭，不取消已接纳的普通 Run。
- `UserPaths` 按 Host 布局生成默认文件权限规则；Host 文件入口、结构化文件 SPI、图片工具授权和 Workspace 注册共用 Runtime 的规则匹配器，禁止再为用户目录增加独立授权分支。按路径组件及真实目标保护兄弟用户、Host 配置、发现文件、备份和 TLS 私钥，公共 Skill 只读；当前生效及待重启私钥随管理配置更新。递归搜索在 rg 遍历前排除保护树，不读取后再过滤结果；Shell/PTY/MCP 仍具备进程的操作系统权限。
- Cookie 业务请求携带 `x-ez-login-context`，浏览器直接 GET 资源可用同名语义的 `login_context` query，PTY Open 首帧携带 `login_context`；该标识不是凭据。已固定身份的 Bearer 不需要此提示。原生路径还要求真实 loopback 和本机 bootstrap 证明；用户凭据选择用户域，`x-ez-host-bootstrap` 仅证明本机管理来源。
- PTY 的 TerminalOrigin 固定用户键；相同 Session/Workspace ID 的删除清理不能跨用户。退出等待旧域进程关闭，重登的新域不受迟到清理影响。保留原 Host PTY、握手、socket、文件读取额度，Speech 的既有额度由用户服务共用，不新增用户配额。
- MCP 的配置、凭据、连接与默认进程目录按用户装配。Skill 在装配时给出有序来源，工作区优先于个人，Host 公共目录为补充；仅个人装配保留旧 OS Home 来源，扫描按 canonical 根去重。Gateway 与 Speech 继续使用本人配置、Runtime、设备登记和生命周期；不增加企业业务禁用清单。

## v0.27.0 C03 M4 客户端管理配置

- HostAccess 状态返回已保存的模式、中心地址/绑定及重启要求；在线与停服入口复用同一配置校验。当前进程模式和企业中心仍使用启动时的值，保存不能即时换域。
- 配置中的模式、中心地址及清除绑定字段只允许本机管理证明提交；个人 Web 原有访问设置权限不能由此提升。切换身份配置前先关闭非本地访问，重启后显式重新开启。
- Desktop 业务凭据与 bootstrap 独立；企业用户退出不妨碍本机管理，但 bootstrap 不能自动恢复成企业用户或初始化个人业务域。

## v0.27.0 C04 M2 中心模型接入

- 企业用户域装配唯一 `CenterModelLoader`，初次启动、当前登录变化和显式刷新触发配置获取；并发意图合并，页面取消等待不取消用户域任务。owner 持有加载任务并在关闭时取消、等待收尾，加载器不另存有效配置。
- 获取时检查中心能力、协议版本、中心身份与同源固定代理路径；网络获取不持身份锁，发布在 Runtime 门禁内核对当前登录及用户域，拒绝迟到结果。
- 企业 Transport 复用既有 Adapter，每次发送取得当前 key，禁用重定向与底层隐式重试；普通响应交原 Adapter。只有中心标记及已知错误码触发控制处理：配置拒绝失效匹配 Arc 并刷新但不重发模型请求，凭据拒绝仅清除匹配的当前登录。
- 个人模式仍使用本地模型工厂；企业模式的主调用、标题、压缩及 child 共用 Runtime 外部准备路径，不回退个人 Provider。

## v0.27.0 C05 子任务窗口存储

现有 v0.27.0 迁移在 `child_tasks` 增加可空、非零正整数 `context_window_tokens`；新任务写入实际装配窗口，旧行保留 NULL。不得以当前模型配置回填，不持久化已用 token 或百分比。升级沿用现有独立备份与事务校验流程；未发布版本的中间数据库不建立兼容迁移。
