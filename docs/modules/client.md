...
- Client 继续通过在线 Host 命令或停服管理 IPC 读写访问设置，不直接探测/编辑 host.toml 或个人配置。Host 的停服读取兼容旧根配置；停服写入必须经过同一目录升级入口，不能由 Client 自行搬迁文件。
- Client、随包 Host、Desktop 及共享应用协议版本/兼容下限对齐到 0.27.0；Center/Admin 保持独立版本。M0 仅完成 Host 配置基础，模式/中心的正式交互归 C03 M4，正式 npm/安装包的默认地址接线与平台制品验证归 C06。

## v0.27.0 C03 M4 模式管理

- `config` 通过原有在线/离线 Host 契约预览、保存模式、中心地址与显式中心绑定重置；Client 不直接改 TOML。`status` 区分已保存配置和重启要求，当前运行模式以 capabilities 为准。
- 企业模式的 `web` 直接打开 Host 登录页，无需个人访问密码，不兑换本机管理身份，也不保存企业账号密码；个人快捷打开沿用既有行为。
