...
产品组成与技术分层分开描述：2026-09-10 用户明确 Desktop 与 TypeScript Client 为同级产品，
各自分发包内置 Host，均可独立安装或同机共存；Host 不单独分发。两类产品共用 Host／Runtime
能力，组件清单不等于进程清单，不改变下述 Runtime 业务所有权边界。同一用户、同一 Runtime
Home 默认共享一个实例。Client、Desktop、Host 各自携带当前版本及最低兼容版本，同一发布版本
使用一致的下限定义，且该下限只表达应用协议兼容性。每次连接初始化检查双方当前版本均达到
对端下限；不满足时当前客户端启动／连接失败，已有 Host 及任务保持运行。Client 装配、现有
协议版本字段职责及具体生命周期路径由 v0.25.2 方案明确；该方向尚未实现。

用户进一步明确 Client 必须可独立替换技术架构。Host、Runtime、Desktop 和共享协议不得
反向依赖 Client 的源码、构建流程、框架、私有状态或随包 Node；共享协议与发布生成层独立于
Client。Host 及系统服务的运行不要求 Client 存在或常驻，Desktop 自行连接与管理 Host。
Client 内部改造不改变共享配置／数据所有权，不因框架变化提高兼容下限；独立性验收见
[v0.25.2 技术方案](../versions/v0.25.2/技术方案.md)。

同日用户补充：数据库独立持久保存最低兼容 Host 版本，与应用协议下限分开维护。Host 在
实例锁内先只读核查，达到数据库要求且通过存储检查后才允许初始化写入和业务服务；不兼容
时保留数据并报告失败。v0.25.2 M1 已实现 Host 私有兼容表和迁移衔接代码，macOS 隔离存储及真实旧 Host 回退验证已通过，M1 已确认。
...
系统服务属于平台生命周期能力，不能与独立 Runtime 进程混为一谈。v0.25.2 已确认提供默认关闭、
可显式配置的自启服务，主要面向 Linux 服务器，由 Client 本机平台适配负责；具体方式与崩溃拉起
策略归技术方案，不进入 Runtime 业务状态。
