...
- 即使函数只具有 `pub(crate)`、`pub(super)` 或私有可见性，只要它跨越持久化事实与内存投影、
  两个 owner/Session、事务提交与事件发布、恢复与正常执行等阶段，就必须用 rustdoc 说明：调用前
  哪些事实已经可靠完成、本函数实际修改什么和明确不修改什么、幂等或重试语义、获取和释放哪些
  gate/锁，以及可靠提交后本函数失败时由何种恢复路径收敛。不能要求维护者仅从 `project`、
  `apply`、`settle`、`recover` 等动词猜测执行阶段。
- 非局部的锁顺序或可靠提交顺序必须同时在负责该顺序的调用点留下简短注释；被调函数的 rustdoc
  不能替代调用点对“为什么先释放 A、再获取 B”的说明。
...
- 跨持久化/投影、跨 owner 或恢复边界的内部函数是否写清前置可靠事实、职责排除、幂等和锁顺序；
