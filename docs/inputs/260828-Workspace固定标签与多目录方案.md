...
- 目标版本：`v0.22.0`
- 目标方向：为 Workspace 增加**固定可读标签（Label）**，并支持**一个 Workspace 下挂多个目录**；同时让 Agent 能感知该关联。

> 正式范围与决策以 [`v0.22.0 功能设计`](../design/v0.22.0-功能设计.md) 为准；本文继续保留现状调研
> 与早期方案推导，不提前敲定表结构、字段类型或迁移脚本。
